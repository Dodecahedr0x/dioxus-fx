...
/// Reads a source file with its line endings normalised to `\n`.
///
/// Git checks these files out with CRLF endings on Windows, and the scans below
/// match against `\n`. Without this they silently find nothing and every check
/// built on them passes vacuously — or, where a count is asserted, fails on a
/// platform difference that has nothing to do with the code under test.
