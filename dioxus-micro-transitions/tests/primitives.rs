...
fn every_class_matches_both_attribute_spellings() {
    ...
    for class in INHERITING.iter().chain([&AMT_COLLAPSE]) {
        for selector in [
...
fn content_classes_also_match_through_an_open_ancestor() {
    ...
    // match on the ancestor's state. `AMT_COLLAPSE` is deliberately not in this
    ...
    assert!(
        !css.contains(&format!("[data-state=open] .{AMT_COLLAPSE}")),
        "{AMT_COLLAPSE} should not match through an ancestor"
    );
}
...
fn both_halves_hold_their_final_frame() {
    ...
    for line in css
        .lines()
        .filter(|line| line.contains("animation:amt-state-"))
    {
        assert!(
...
fn the_positioning_transform_is_left_alone() {
    ...
    for keyframes in css.split("@keyframes amt-state-").skip(1) {
...
fn reduced_motion_shortens_rather_than_removes() {
    ...
    let rule = ".amt-state-fade,.amt-state-zoom,.amt-state-slide,.amt-state-menu,.amt-state-collapse{--amt-enter:.01ms;--amt-exit:.01ms}";
    let at = css
        .find(rule)
