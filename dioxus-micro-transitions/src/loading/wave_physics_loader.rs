...
.amt-wave-physics{display:flex;flex-direction:column;align-items:center;justify-content:center;transform:scale(var(--amt-scale));transform-origin:center}
.amt-wave-physics div{position:relative;display:flex;align-items:flex-end;justify-content:flex-start;gap:8px;width:292px;height:192px}
.amt-wave-physics i{width:12px;height:16px;border-radius:9999px;background:var(--amt-track);transform-origin:bottom center;animation-duration:var(--amt-duration);animation-timing-function:linear;animation-iteration-count:infinite}
.amt-wave-physics b{position:absolute;bottom:0;left:0;z-index:1;width:12px;height:12px;border-radius:9999px;background:var(--amt-color);transform-origin:bottom center;animation:amt-wave-physics-ball var(--amt-duration) linear infinite}
"#;
...
pub(crate) fn keyframes() -> &'static str {
    ...
    CACHE.get_or_init(|| {
        let mut css = String::from("@keyframes amt-wave-physics-ball{");
        ...
        for i in 0..BARS {
            css.push_str(&format!("@keyframes amt-wave-physics-bar-{i}{{"));
            prev.clear();
            for k in 0..FRAMES {
                let t = k as f64 / (FRAMES - 1) as f64;
                let (height, wave) = bar_at(i, t);
                let body = format!(
                    "height:{height:.1}px;background:color-mix(in srgb,var(--amt-color) {:.0}%,var(--amt-track))",
                    wave * 100.0
                );
                if body != prev || k == FRAMES - 1 {
                    css.push_str(&format!("{:.2}%{{{body}}}", t * 100.0));
            ...
            css.push('}');
            css.push_str(&format!(
                ".amt-wave-physics i:nth-child({}){{animation-name:amt-wave-physics-bar-{i}}}",
                i + 1
            ));
        }
        css
...
pub fn WavePhysicsLoader(
    ...
    rsx! {
        document::Style { href: "amt:base", {BASE_CSS} }
        document::Style { href: "amt:wave-physics-loader", {CSS} }
        document::Style { href: "amt:wave-physics-loader-keyframes", {keyframes()} }
        div {
            class: "amt amt-loader amt-wave-physics {class}",
            style: "--amt-scale:{scale};--amt-color:{color};--amt-duration:{duration}s;",
            role: "status",
            aria_label: "Loading",
            ..attributes,
            div {
